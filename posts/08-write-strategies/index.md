...
**Decay.** The simplest scheme that works: drop `confidence` by a small amount on every day without access; demote to a colder tier (or delete) below a threshold. The agent should *ask before asserting* anything below some cutoff (a value near 0.6 is a common starting point, illustrative rather than a measured constant; tune it against real behaviour).
...
**A worked `task.md` lifecycle.** The plan-plus-checklist pattern is worth seeing concretely, because coding agents such as Claude Code lean on it heavily. The agent writes a single `task.md`, then re-reads and edits it every iteration. A fresh one might look like this:
...
- [ ] Add a token-bucket limiter (5 attempts / 15 min / IP)
- [ ] Return 429 with a Retry-After header
- [ ] Add a test for the limit and the reset

## Notes
(empty)
```

After a couple of tool calls the same file has grown a memory of what happened, so the agent never has to reconstruct it from conversation history:

```markdown
# Task: add rate limiting to the /login endpoint

## Plan
- [x] Find where /login is defined  → routes/auth.py:42
- [x] Add a token-bucket limiter (5 attempts / 15 min / IP)
- [ ] Return 429 with a Retry-After header
- [ ] Add a test for the limit and the reset

## Notes
- Existing middleware stack is in app/middleware.py; add the limiter there,
  not in the route, so it also covers /login/mfa.
- Redis is already a dependency → use it for the bucket, not in-process state.
```

The lifecycle is three moves repeated until the boxes are ticked: **read** the file to recover state, **act** on the first unchecked item, **write** back the result and any note the next iteration will need. Because the plan lives in a file rather than the transcript, a long task can run for dozens of turns without the early context scrolling out of the window. When the task finishes, the file is either deleted or promoted: a durable decision ("use Redis, not in-process state") belongs in `AGENTS.md`, not in a scratch file that vanishes with the task.
...
`AGENTS.md` (an open, cross-vendor convention with published support from several coding-agent tools, including Codex, Cursor, Aider, and Continue; agents.md project, 2025) and the platform-specific `CLAUDE.md` (Anthropic's Claude Code) are plain Markdown files that live alongside the code. The host loads them at session start and prepends the relevant sections to every call. The files are *checked into the repository*: they are reviewed, version-controlled, and diffed like code.

The two files play the same role but differ in scope. `AGENTS.md` is the vendor-neutral format that any conforming tool reads; `CLAUDE.md` is Claude Code's own file, read only by Claude Code, and is the place to put Claude-specific instructions (slash commands, Anthropic tool conventions). A project that wants both portability and Claude-specific tuning keeps a shared `AGENTS.md` and a thin `CLAUDE.md` that references it:

| | `AGENTS.md` | `CLAUDE.md` |
|---|---|---|
| Read by | any conforming agent (Codex, Cursor, Aider, Continue, …) | Claude Code only |
| Scope | vendor-neutral project rules | Claude-specific instructions |
| Best for | portable build/test/style rules | slash commands, Claude tool conventions |
| Recursion | nearest file wins per directory | nearest file wins per directory |
...
A typical `AGENTS.md` has five blocks (the same five as the system prompt, [Post 14](../14-system-prompt-as-software/index.md)):
...
5. **Tools / commands**: slash commands the agent can invoke, MCP servers (Model Context Protocol, the open standard for exposing tools to agents; [Post 15](../15-tools-and-mcp/index.md)) configured for this project.

**`skill.md` files** are a more specialised variant: a single skill (for example, "review a Terraform diff" or "draft a PR description in the project's format") packaged as a Markdown file with its own rules and few-shot examples, loaded only when the agent decides it needs that skill. By convention these live together in a `.skills/` directory, one file per skill, each in its own subfolder alongside any scripts or templates it needs; the host indexes their names and descriptions and pulls in the full file only when a task matches. The value is partly the content and partly the *pattern* (one file per concern, loaded conditionally) rather than one giant always-on prompt.

Three rules of thumb that come from teams running these in production:

- **Keep the file short** (a few hundred lines is a useful ceiling, illustrative rather than a hard limit). The host is going to load most of it on every relevant call, so a sprawling `CLAUDE.md` makes the cached prefix huge and the system prompt unreadable to humans.
...
**Spec-driven development.** The strongest form of this discipline pushes the checked-in file *upstream of the code*. Instead of prompting an agent turn by turn, the operator writes a specification (a plan file describing what to build, the constraints, and the acceptance tests), reviews and edits that spec like any other artefact, and only then hands it to the agent to implement. The spec is a write destination in exactly the sense of this post: durable, versioned, reviewed, and re-read on every relevant call. It differs from a `task.md` scratchpad in lifetime and audience. A scratchpad is the agent's private working memory for one task and is thrown away; a spec is a human-authored contract that outlives the run and can be re-implemented, diffed, or handed to a different agent. In practice the two chain together: a reviewed spec generates a `task.md` plan, the agent works the plan, and durable decisions flow back into `AGENTS.md`. The pattern matters because the review happens on the *specification*, where a mistake costs one edit, rather than on a thousand lines of generated code, where it costs a re-run.

---

## 5. Embedding indexes: the offline write side of RAG

Every RAG system (retrieval-augmented generation: fetch relevant text at query time and add it to the prompt) has two halves: an *online* read path that retrieves at inference time (covered in [Post 11](../11-rag-in-depth/index.md)), and an *offline* write path that prepares the corpus. The write side is where the quality is decided.
...
3. **Chunking.** Split into chunks of 400–600 tokens for prose (200–300 for code), with 10–20% overlap (roughly 50–100 tokens) between adjacent chunks. These are working defaults, not measured constants; sentence-aware splitters beat fixed-length ones for prose, and code is best split by symbol.
4. **Enrichment.** Prepend a short *contextual header* to each chunk (the document title and section path, sometimes a 1–2 sentence summary of the parent document; Anthropic's "contextual retrieval" trick). This single step reduces retrieval failures by about 35% on its own, about 49% when paired with a contextual BM25 keyword index (BM25 is the classic term-frequency ranking function used by keyword search), and about 67% once a reranker is added (Anthropic, 2024).
...
- LangChain Blog, *"Memory for Agents"* (2024): the episodic / semantic / procedural taxonomy used in §2.
- Anthropic Engineering, *"Building Effective AI Agents"* (2024): workflow and agent patterns, including scratchpad-style working memory.
- Anthropic, *"Introducing Contextual Retrieval"* (2024): the ~35% / 49% / 67% failure-reduction figures and the chunk-enrichment trick in §5.
- agents.md project, *"AGENTS.md — a simple, open format for guiding coding agents"* (2025): the cross-vendor convention in §4.
...
- **[Post 07 — Write, Select, Compress, Isolate](../07-write-select-compress-isolate/index.md)**: back to the framework this post's *Write* operation sits inside.
- **[Post 09 — Select strategies](../09-select-strategies/index.md)**: the read-path counterpart to everything in this post.
- **[Post 16 — Memory systems](../16-memory-systems/index.md)**: the in-prompt memory layer, in depth.
- **[Post 14 — The system prompt as software](../14-system-prompt-as-software/index.md)**: how `AGENTS.md` and `CLAUDE.md` get treated like code.
