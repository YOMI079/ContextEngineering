...
A naïve reading of the WSCI framework treats Select and RAG (retrieval-augmented generation: fetching relevant text at query time and adding it to the prompt) as synonyms. They are not. **RAG is one application of Select**; Select is the operation. Once you can see the family resemblance, three things stop being separate problems:

- *Tool selection*: out of 100 tools, which 5 schemas should this turn see? ([Post 15](../15-tools-and-mcp/index.md))
- *Memory recall*: out of thousands of stored facts about this user, which handful go in this prompt? ([Post 16](../16-memory-systems/index.md))
...
Two numbers run through the whole pipeline. **Top-N** is the *candidate-set size*: how many plausibly-relevant items candidate generation retrieves (illustratively ~50). **Top-k** is the *packed count*: how many survive re-ranking and actually land in the prompt (illustratively ~5). N is tuned for recall; k is bounded by budget. ANN here is *approximate nearest-neighbour* search: the fast, sub-linear vector lookup that makes dense retrieval tractable.
...
**HyDE (Hypothetical Document Embeddings).** Counter-intuitively, retrieval can improve if you ask the model to write the *answer* it expects, then embed *that* and retrieve against it. The model's hypothetical answer is wrong in detail but right in vocabulary, and vocabulary is what the embedding model matches on. HyDE costs one extra LLM call; the original paper reports consistent zero-shot recall gains over a plain query embedding on several retrieval benchmarks (Gao et al., 2022), and it tends to help most on technical or jargon-heavy corpora.
...
**Sparse retrieval.** Classical BM25 (Best Match 25, the standard TF-IDF-family lexical ranking function) over a Lucene-family index (Elasticsearch, OpenSearch, tantivy). Strong on exact-token and rare-term matches. Weak on paraphrase ("car" vs. "automobile").
...
**Hybrid retrieval** combines the three by running them in parallel and merging the result sets. The merger is usually **reciprocal rank fusion** (RRF): each candidate's final score is the sum of the reciprocals of its ranks across the result lists. RRF is effectively hyper-parameter-free and robust, and in its original evaluation it outperformed both individual rank-learning methods and Condorcet fusion (Cormack et al., 2009); in practice it reliably beats either dense or sparse retrieval used alone.

A reasonable default for a new system: dense (top-30) + BM25 (top-30) + metadata filter, fused by RRF, into a top-50 candidate set.

One offline trick sharpens both halves of the hybrid at once. **Contextual Retrieval** (Anthropic's name for it) prepends each chunk, before indexing, with a short model-written summary of the document it came from, so the chunk carries its own context into both the embedding and the BM25 index. Anthropic reports ~35 % fewer retrieval failures with contextual embeddings alone, ~49 % when contextual BM25 is added, and ~67 % once a reranker is layered on top (Anthropic, 2024). It is one of the highest-leverage things you can do to candidate generation; [Post 11](../11-rag-in-depth/index.md) implements it.
...
The dominant technology is the **cross-encoder**: a small model that takes the query and one candidate at a time and outputs a relevance score. Cross-encoders are slow per pair but brilliant per pair, because they get to see the query and the chunk *together*, which a bi-encoder embedding match cannot do. As an order-of-magnitude illustration: if scoring one pair takes tens of milliseconds, scoring 50 candidates is around a second, while scoring a whole million-item corpus would run into hours. That gap is exactly why candidate generation exists: it shrinks the set the expensive re-ranker has to look at.
...
The lift from re-ranking is often the biggest single quality jump in the entire pipeline. Going from "top-5 by ANN distance" to "top-5 by cross-encoder over top-50 candidates" is frequently the largest single reduction in wrong-source answers a pipeline will see. Anthropic's Contextual Retrieval study puts a number on the reranking step specifically: adding a reranker on top of contextual embeddings and contextual BM25 cut retrieval failures by ~67 %, versus ~49 % without the reranker (Anthropic, 2024). Post 11 covers that technique end to end.
...
**Cite as you pack.** Format each chunk with a stable, model-readable header (`[source: docs/refunds.md, chunk 3]`). When instructed to, the model will quote those headers back in the answer, giving the application a citation it can render to the user and a debug trail it can grep. This is not automatic and not perfectly reliable: models can attach the wrong header or invent one, so citation accuracy is something to *measure*, not assume (see §8 and [Post 20](../20-evaluation/index.md)).
...
**Few-shot example selection.** The corpus is the labelled-example bank. Each example is one document; the metadata records its label class. Retrieval is dense + filtered ("only examples of class C"). As a rule of thumb, pack two or three examples: beyond a handful, extra examples mostly add cost while quality flattens, so the marginal example rarely pays for its tokens. The trick that distinguishes good few-shot pipelines is *diversity reranking*: pick examples that are individually relevant *and* collectively distinct, so the model sees varied templates rather than three near-identical ones. The usual mechanism is maximal-marginal-relevance (MMR): when choosing the next example, penalise each candidate by its similarity to the ones already picked, so redundant near-duplicates lose out to a slightly-less-relevant but more distinct example.
...
5. **Chunk size.** 400–600 tokens is a strong default for prose; 200–300 for code, with 10–20 % overlap (roughly 50–100 tokens). Tune by measuring the rate at which the answer's supporting fact is *fully inside* the retrieved chunk. The chunking *strategy* itself (fixed, recursive, semantic, parent-child, late chunking) is a topic in its own right and is covered concretely in [Post 11](../11-rag-in-depth/index.md).
6. **Embedding model.** Switch only with an offline benchmark. As of early 2026 a solid, cheap general-purpose tier is a small OpenAI `text-embedding-3` model, a small Voyage embedding model, or an open-weight `bge-large` (check each provider's current model list, as names and versions turn over quickly). That tier is good enough for almost everything; the gains from re-ranking usually dwarf the gains from swapping embedders.
...
Adding a third, **end-to-end answer correctness on a held-out eval set**, closes the loop, and is the subject of [Post 20](../20-evaluation/index.md).
...
- **Tuning embedding models before tuning chunking.** Chunking usually has far more headroom than the embedder; fix how the corpus is split before shopping for a new embedding model.
...
- Anthropic Engineering, *"Contextual Retrieval"* (2024): chunk-enrichment + hybrid + re-ranking; source of the ~35 / 49 / 67 % figures.
- Carbonell, J. & Goldstein, J., *"The Use of MMR, Diversity-Based Reranking for Reordering Documents and Producing Summaries"* (1998): maximal-marginal-relevance.
- Cohere, *"Rerank"* docs; Voyage AI, *"Rerankers"* and *"voyage-3"* model cards; BAAI, *"bge-reranker-v2"* model card.
...
- **[Post 11 — RAG in depth](../11-rag-in-depth/index.md)**: the document-corpus instance, with concrete code.
- **[Post 15 — Tools and MCP](../15-tools-and-mcp/index.md)**: the tool-corpus instance.
- **[Post 16 — Memory systems](../16-memory-systems/index.md)**: the memory-corpus instance.
- **[Post 17 — Advanced retrieval](../17-advanced-retrieval/index.md)**: graph RAG, late-interaction models, structured retrieval.
