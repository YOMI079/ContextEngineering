...
Before the model sees anything, the input string is broken into **tokens** by a deterministic tokeniser, almost always a byte-pair encoding (BPE) variant. A token is roughly three quarters of an English word; the exact ratio depends on language, code, and punctuation. The string `"context engineering"` is two tokens for GPT-4o, three for Claude 3.5, and seven for an older Llama tokeniser. The first practical consequence is that **counting words is not counting cost**. The next post is dedicated to this; for the rest of *this* post we treat tokens as the atomic unit.

Each token is then mapped to a learned **embedding vector**, typically 4 096 to 12 288 dimensions in modern frontier models. Stack the embeddings for an *N*-token input and you get a matrix of shape `[N, d]`. Everything the model does internally is a function of that matrix and the (very large) parameter tensors of the network. There is no separate "memory" the model consults; its only input is this matrix, and its only output is a distribution over the next token.
...
2. **Compress the middle.** If your retrieval, history, or memory layers grow past a few tens of thousands of tokens, reach for compression (Posts 10 and 14) before you reach for a bigger window.
...
That is the *intra-request* KV-cache. The interesting part for context engineering is its *inter-request* cousin: **prompt caching**, sometimes called prefix caching. If two API calls share the same first *k* tokens, byte-for-byte, the provider can reuse the KV-cache it computed for those tokens on the previous call. You are billed at a heavily discounted rate (Anthropic and OpenAI publish ~10× cheaper input pricing for cache hits) and latency drops correspondingly.
...
- **Caches expire.** Most providers hold a cache for five minutes of inactivity. High-throughput agents naturally hit warm caches; low-traffic side projects rarely do. Post 19 quantifies the trade-off.

---

## 7. Needle-in-a-haystack, and what it does and does not measure

Vendors love to publish "needle-in-a-haystack" results: hide a single sentence (the *needle*) inside a million-token document (the *haystack*), ask the model to retrieve it, and report accuracy. Modern flagship models score above 95 % on these tests well into the 1–2 M token range.

Two things are important to keep in mind when reading these charts.

First, **needle tests measure recall, not reasoning.** Finding a single planted fact is the easiest thing a long-context model can be asked to do. It does not measure whether the model can *combine* facts spread across the haystack, *notice contradictions*, or *use* the recovered information to answer a non-trivial question. Independent benchmarks (RULER, LongBench, BABILong) routinely show order-of-magnitude drops in performance the moment the task moves from "find the fact" to "use the facts".
...
The operational rule: trust your own evals at your own context length, not the vendor's needle chart. Post 16 covers eval design.
...
- Anthropic Engineering, *"Prompt caching with Claude"* (2024): the canonical write-up of prefix caching, with pricing.
...
- **[Post 05 — Five context failure modes](../05-context-failure-modes/index.md)**: what happens when the placement rules above are violated.
- **[Post 19 — Long context vs RAG](../19-long-context-vs-rag/index.md)**: when the production economics tip toward RAG instead of stuffing.
