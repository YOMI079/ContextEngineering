...
![Timeline of the field from the 2022 prompt era through the 2024 RAG era to the 2025 agent era.](diagrams/01-timeline.svg)
*Three years, three eras: the surface the engineer works on grows from a single message to a whole window.*
...
**First, models gained tools.** Function calling shipped in mid-2023; within a year the major providers supported it natively; by 2025 the Model Context Protocol (an open standard for exposing tools to models, covered in Post 15) made tool definitions portable across hosts. The model now sees, on every call, the JSON schema of every tool it might use, sometimes thousands of tokens before the user's message even appears.
...
**Third, models gained memory.** The conversation buffer is the obvious form, but persistent memory (episodic recalls of past sessions, semantic facts kept in a project memory file, procedural skills stored as reusable instructions) is now standard in serious agent systems. None of this is the user's prompt; all of it shapes the answer.

Add the system prompt and the conversation history that already existed, and the six layers of a modern call come into view: system prompt, tool definitions, memory, retrieved context, conversation history, and the user's message. The user's literal sentence is a small fraction of what the model reads. A modern call is not a prompt being engineered; it is a **window** being assembled.

![Venn diagram: the user's prompt is a small subset of the full context the model reads.](diagrams/02-prompt-subset.svg)
*The prompt is a slice of the context, not the whole of it, which is exactly what the rename acknowledges.*
...
**Lance Martin** wrote the influential "Context Engineering for Agents" essay (LangChain blog, 2025), which named the four primitives this series uses throughout: **Write, Select, Compress, Isolate**.

**Dex Horthy's** "12-Factor Agents" project, drawn from interviews with practitioners, made the practical case that a large share of production-grade agent failures trace back to a context decision rather than a model limitation.
...
**The window is finite, and the cost is real.** A million-token context window sounds infinite until it starts filling with tools, retrieved documents, and history. At Claude Sonnet 4.5 input rates (about three US dollars per *million* tokens as of early 2026), a single 100 000-token uncached call costs roughly thirty cents; multiply by ten thousand daily users and the bill runs to thousands of dollars a day. Engineering decisions (what to include, what to compress, what to write to disk) now translate directly into latency and money. (Post 04 works the arithmetic in full.)
...
![Icon grid of the five jobs of a context engineer: decide what goes in, what stays out, where each piece sits, when to refresh, and how to measure.](diagrams/03-five-jobs.svg)
*Every context-engineering task is one of these five jobs, whatever the application.*
...
**2. Decide what stays out.** The harder, more important sibling of the first job. Every token that does not need to be there competes for the model's attention with the tokens that do. Pruning is not optimisation; it is correctness work. (Posts 06 and 12.)

**3. Decide where each piece sits.** Position matters. The model attends more strongly to the start and end of a long context than to the deep middle — because attention over a long sequence is not uniform (the mechanism is Post 03's subject). Cacheable layers must come first or they cannot be cached. The user's instruction usually belongs last, on purpose. (Post 03.)

**4. Decide when to refresh, compress, or write out.** A long-running session accumulates context; left alone, it rots. Engineers schedule compaction, summarisation, and externalisation of state to disk. (Post 12, Post 16.)

**5. Decide how to measure whether it worked.** Without traces, evals, and cost dashboards, every change is a guess. The same change that improves answers on one slice of traffic can silently degrade another. (Post 22.)
...
- Lance Martin, "Context Engineering for Agents" (LangChain blog, 2025) — the WSCI essay.
- Dex Horthy, "12-Factor Agents" — practitioner patterns from production agent failures.
- Anthropic, "Pricing" (per-million-token rates, current as of early 2026) — the source for the cost figure in §3.
...
- **[Post 07 — Write, Select, Compress, Isolate](../07-write-select-compress-isolate/index.md)**: if you prefer to start with the verbs instead of the nouns.
